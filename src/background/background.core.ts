...
export const init = async (): Promise<void> => {
   chrome.alarms.create({ periodInMinutes: DEFAULT_ALARM_PERIOD_IN_MINUTES });
   setData();
};
...
const setData = async (): Promise<void> => {
   ...
   await setLocalStorage(LOCALSTORAGE_DATE, "Actualizado hace " + lastUpdated);
...
const generateTooltip = async (exchangeRate: ExchangeRate): Promise<string> => {
   ...
   return tooltip
      .replace("{0}", exchangeRate.name)
      .replace("{1}", formatCurrency(exchangeRate.buy).toString())
      .replace("{2}", formatCurrency(exchangeRate.sell).toString())
      .replace("{3}", sinceDate);
};
...
const setConfig = async (): Promise<Config> => {
   ...
   return localConfig ? new Config(
      ...
      localConfig.exchangeRateTypeToShowInBadge ??
         DEFAULT_BADGE_EXCHANGE_RATE_TYPE,
      localConfig.exchangeRateToShowInTooltip ?? DEFAULT_TOOLTIP_EXCHANGE_RATE,
      localConfig.conversor ?? DEFAULT_CONVERSOR,
      localConfig.conversorType ?? DEFAULT_CONVERSOR_TYPE,
      localConfig.conversorAmount ?? null,
      localConfig.alarmPeriodInMinutes ?? DEFAULT_ALARM_PERIOD_IN_MINUTES,
