...
const Options = () => {
		...
		if (exchangeRates) {
			exchangeRates.map((item: any, i: number) => {
				...
				if (item.id === localConfig.exchangeRateToShowInTooltip) {
					...
					const tooltipString: string = chrome.i18n.getMessage(
						"tooltip",
						[
							item.name,
	...
	const validateConfig = (configParam) => {
		return configParam
			? !(
					...
					configParam.hideConversionSection ===
			...
			setBadgeExchangeRatesList(
				exchangeRates.map((item) => {
					return {
						...
						label: item.name,
						isSelected:
							config.exchangeRateToShowInBadge === item.id,
					};
				})
			);
			setTooltipExchangeRatesList(
				exchangeRates.map((item) => {
					return {
						...
						label: item.name,
						isSelected:
							config.exchangeRateToShowInTooltip === item.id,
					};
				})
			);
			setSellBuyList([
	...
	return (
		<div className="dark:bg-blue-gray-900 bg-[#EEF3F7]">
			...
			<div className="w-2/5 h-screen mx-auto mt-12">
				<div className="flex-col flex">
					...
					<Container>
						<div className="w-full flex justify-center">
							<a
								href="https://www.buymeacoffee.com/mauropitu"
								target="_blank">
								<img src={bmac} className="w-32 rounded-lg" />
							</a>
						</div>
					</Container>
				</div>
